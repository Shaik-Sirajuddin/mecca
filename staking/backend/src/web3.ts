...
export const findTotalAccInterest24Hrs = async (
  ...
  appState: AppState,
  programAccounts: GetProgramAccountsResponse
) => {
  try {
    let userDataPDAs = programAccounts;
...
export const getPendingPrinciapalWithdrawl = async (
  programAccounts: GetProgramAccountsResponse
) => {
  try {
    let userDataPDAs = programAccounts;
    ...
    for (let i = 0; i < userDataPDAs.length; i++) {
      ...
      if (
        ...
        pendingAmount = pendingAmount.add(
          user.withdraw_request.amount.toString()
        );
      }
    }
    return pendingAmount;
  } catch (error) {
    console.log("Calculate pending principal error", error);
    return new Decimal(0);
  }
};
